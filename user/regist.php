<?php
	...
	if (empty($error) && empty($usernameError) && empty($passwordError) && empty($phoneError)) {
		...
		$sql = "select `id` from `t_users` where `phonenumber`='$phone'";
		$rst = db_query($sql);
		if (mysqli_fetch_row($rst)) {
		    $phoneError[] = '该号码已被注册过。';
		    showRegPage();
		...
		$sql = "insert into `t_users`(`username`,`password`,`phonenumber`) values('$username','$password','$phone')";
		$rst = db_query($sql);
		if ($rst) {
		    session_start();
		    ...
		    $userid = $row['id'];
		    $cartinfo = $row['id'].'cartinfo';
	        $_SESSION[$cartinfo] = array(array());
		    $sql = "insert into `t_user_information`(`userid`) values('$userid')";
		    $rst = db_query($sql);
		    alert_back('注册成功！');
		    exit;
		} else {
